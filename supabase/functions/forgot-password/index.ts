    ...
    if (resendApiKey) {
      try {
        ...
        if (!emailResponse.ok) {
          ...
          console.error('Resend API error (email not sent, but reset link created):', emailResponse.status, errorData);
        } else {
          const emailResult = await emailResponse.json();
          console.log('Password reset email sent successfully:', emailResult);
        }
      } catch (emailError) {
        console.error('Email sending failed (reset link still valid):', emailError);
      }
    }
