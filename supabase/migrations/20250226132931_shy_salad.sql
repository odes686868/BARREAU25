/*
  # Create unified questions table with proper structure
...
-- Create unified questions table with all fields
...
-- Function to convert old incorrect answers format to array
CREATE OR REPLACE FUNCTION convert_incorrect_answers(
  ans0 text,
  ans1 text,
  ans2 text
) RETURNS text[] AS $$
BEGIN
  RETURN ARRAY[ans0, ans1, ans2]::text[];
END;
$$ LANGUAGE plpgsql;
